<?php
/**
 * FILE:    schedule_officer/reports/asset_summary_usd/index.php
 * PURPOSE: Asset Summary (USD) — one row per class, pool + individual,
 *          translated at historical rate per item (IAS 21).
 *
 * REPLACES the old version whose SQL referenced columns that do not exist
 * (open_bal_usd, total_accum_start_usd, …) and so always returned an error.
...
<head>
  ...
  <title>Asset Summary (USD) — RMU Asset Register</title>
...
<header class="report-topbar no-print">
  ...
  <div style="flex:1">
    <h1 style="font-size:1rem;font-weight:700;margin:0">Asset Summary — Movement (USD)</h1>
    <small class="text-muted">Pool + individual · historical rate per item (IAS 21) · active rate $<?= number_format($activeRate,2) ?></small>
  </div>
...
<main class="rmu-content-full">
  ...
  <?php if (!$rows): ?>
    <div class="rmu-card"><div class="rmu-card-body"><div class="rmu-empty">
      <i class="bi bi-inbox rmu-empty-icon"></i><h6>No assets in service for <?= $selectedYear ?></h6></div></div></div>
  <?php else: ?>
    <div class="kpi-grid section-gap" style="grid-template-columns:repeat(4,1fr)">
      <div class="kpi-card kpi-blue"><div class="kpi-label">Total Cost</div><div class="kpi-value num" style="font-size:1.3rem">$ <?= usd($grand['cost']) ?></div><div class="kpi-sub"><?= $grand['count'] ?> assets · <?= count($rows) ?> classes</div></div>
      <div class="kpi-card kpi-amber"><div class="kpi-label">Depr. Charge <?= $selectedYear ?></div><div class="kpi-value num" style="font-size:1.3rem">$ <?= usd($grand['charge']) ?></div><div class="kpi-sub">Expense for the year</div></div>
      <div class="kpi-card kpi-slate"><div class="kpi-label">Accum. Depreciation</div><div class="kpi-value num" style="font-size:1.3rem">$ <?= usd($grand['accum_end']) ?></div><div class="kpi-sub">Carried forward</div></div>
      <div class="kpi-card kpi-green"><div class="kpi-label">Net Book Value</div><div class="kpi-value num" style="font-size:1.3rem">$ <?= usd($grand['nbv']) ?></div><div class="kpi-sub">Closing</div></div>
    </div>

    <div class="rmu-table-wrapper">
      <table class="rmu-table sum-table" id="summaryTable">
        <thead><tr>
          <th>#</th><th>Asset Class</th><th class="num">Assets</th><th class="num">Cost</th>
          <th class="num">Accum. Depr. b/f</th><th class="num">Charge (<?= $selectedYear ?>)</th>
          <th class="num">Accum. Depr. c/f</th><th class="num">Net Book Value</th>
        </tr></thead>
        <tbody>
          <?php $sn=1; foreach ($rows as $r): ?>
            <tr>
              ...
              <td><?= htmlspecialchars($r['name']) ?>
                <?= $r['wip']?'<span class="status-pill status-warning ms-1">WIP</span>':'' ?>
                <?= $r['sub']?'<i class="bi bi-exclamation-triangle text-warning ms-1" title="includes a substituted rate"></i>':'' ?></td>
              <td class="num"><?= $r['count'] ?></td>
              <td class="num"><?= usd($r['cost']) ?></td>
              <td class="num"><?= usd($r['accum_start']) ?></td>
              <td class="num"><?= usd($r['charge']) ?></td>
              <td class="num"><?= usd($r['accum_end']) ?></td>
              <td class="num"><strong><?= usd($r['nbv']) ?></strong></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
        <tfoot><tr>
          ...
          <td class="num"><strong><?= usd($grand['cost']) ?></strong></td>
          <td class="num"><strong><?= usd($grand['accum_start']) ?></strong></td>
          <td class="num"><strong><?= usd($grand['charge']) ?></strong></td>
          <td class="num"><strong><?= usd($grand['accum_end']) ?></strong></td>
          <td class="num"><strong><?= usd($grand['nbv']) ?></strong></td>
        </tr></tfoot>
      </table>
    </div>
    <p class="text-muted mt-3" style="font-size:.75rem"><i class="bi bi-info-circle me-1"></i>
      Each item translated at its historical rate (IAS 21). <i class="bi bi-exclamation-triangle text-warning"></i> marks a class containing a substituted rate (stored rate missing/invalid → active rate $<?= number_format($activeRate,2) ?>). Consistent with the per-class USD schedule. Computed live; no stored data altered.</p>
  <?php endif; ?>
</main>
