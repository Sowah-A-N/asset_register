...
echo "<tbody>
        <tr>
          ...
          <td class='border px-4 py-2'>" . date('d M Y', strtotime($rowAsset['acquisition_date'])) . "</td>
            ...
            <td>$" . number_format($rowAsset['additions'] /$rate, 2, ".", ",") . "</td>
...
foreach ($filteredResults as $year => $result) {
    echo "<tr>";
    echo "<td>$year</td>";
    echo "<td>$" . number_format(($result['book_value_start']/$rate), 2) . "</td>";
    echo "<td>" . ($result['depreciation_rate'] ) . "</td>";
    echo "<td>$" . number_format(($result['depreciation_expense'] /$rate), 2) . "</td>";
    echo "<td>$" . number_format(($result['accumulated_depreciation']/$rate), 2) . "</td>";
    echo "<td>$" . number_format(($result['book_value_end'] /$rate), 2) . "</td>";
...
if ($selectedYear >= $acquisitionYear) {
    ...
    if ($acquisitionYear == $selectedYear) {
        ...
        for ($month = $acquisitionMonth; $month <= 12; $month++) {
            ...
            echo "<td>$" . number_format($monthly_depreciation /$rate, 2) . "</td>";
            echo "<td>$" . number_format($monthly_accumulated /$rate, 2) . "</td>";
            echo "<td>$" . number_format($monthly_book_value /$rate, 2) . "</td>";
            echo "</tr>";
        }
...
echo "</div>";
echo '<a href="../all_assets_usd/index.php" class="mb-4 inline-block bg-blue-500 text-white px-6 py-2 rounded shadow-md hover:bg-blue-600 transition duration-200">
