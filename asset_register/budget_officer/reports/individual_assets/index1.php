...
$query="SELECT opening_bal,dep_rate FROM asset_classes WHERE asset_class = '$assetClass' ";
$result = $conn->query($query);
$row=mysqli_fetch_array($result);
...
if($result->num_rows > 0){
    ...
    while($row = $result->fetch_assoc()){            
        ...
        $yearAdded = (int)(date("Y",strtotime($row['acquisition_date']))) ?? "No year to show";
        echo "Year Added : " . $yearAdded . "\n" . "<br />";

        $currentYear = (int)date("Y");
        echo "Current year : " . $currentYear . "\n" . "<br />";

        $yearInterval = (int)($currentYear - $yearAdded);
        echo "Year Interval : " . $yearInterval . "<br />";

        $initialValue =  (number_format($row['additions'], 2, ".", ""));
        $currentInitialValue = $initialValue;
        $newValue;
        $numOfYears;
        $depRateInPercent = ($dep_rate * 100);

        echo "Additions : " . $initialValue . "\n" . "<br />";

        for($i = $yearInterval; $i >= 0 ; $i--){               

            // echo $currentYear - $i . "\n" . " : ";
            // $newValue = ($currentInitialValue * (float)((100-$depRateInPercent)/100));
            // echo number_format((float)$newValue, 2, ".", ",") . "\n" . "<br />";
            // $currentInitialValue = $newValue;

            $depAmount = $initialValue * (($depRateInPercent / 100));
           
            while ($i > 0) {
                $currentInitialValue -= $depAmount;
                echo $currentYear - $i . " : " . number_format($currentInitialValue, 2, ".", ",") . "\n" . "<br />";
                $i--;
            }
        }
...
echo "<td class='py-2 px-4 border-b'>" . number_format($opening_bal, 2, ".", ",") . "</td>";
echo "<td class='py-2 px-4 border-b'>" . number_format($assetCostClosingBalance, 2, ".", ",") . "</td>";
echo "<td class='py-2 px-4 border-b'>" . number_format($depreciationCost, 2, ".", ",") . "</td>";
echo "<td class='py-2 px-4 border-b'>" . number_format($totalAccumulatedDepreciation, 2, ".", ",") . "</td>";
echo "<td class='py-2 px-4 border-b'>" . number_format($accountDepreciationClosingBalance, 2, ".", ",") . "</td>";
echo "<td class='py-2 px-4 border-b'>" . number_format($closingCarryingValue, 2, ".", ",") . "</td>";
