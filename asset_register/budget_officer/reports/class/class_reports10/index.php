...
<body>
    ...
    <?php       
        ...
        if (empty($reportYear) || empty($assetClass)) {
            //echo "Please select both year and asset class.";
        } else {
            
            $assetData = calculationsByClass($assetClass, $reportYear);
            //var_dump($assetData);
            displayData($assetClass, $reportYear, $assetData);
        }

        // Create the form
        echo '<form action="generate.php" method="POST" class="d-flex justify-content-center">';
        echo '    <button type="submit" class="btn btn-outline-primary btn-rounded ">' . "Export to PDF" . '</button>';
        echo '</form>';
