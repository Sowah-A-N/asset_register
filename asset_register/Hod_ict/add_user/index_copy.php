...
if (isset($_POST['asset_class'])) {
    // Retrieve the selected asset class value from the AJAX request
    $assetClass = $_POST['asset_class'];

    // Query the database to fetch the corresponding subclasses based on the selected asset class
    $sql = "SELECT * FROM asset_class_sub_classes WHERE asset_class = '$assetClass'";
    $result = mysqli_query($conn, $sql);

    // Generate HTML options for the subclasses
    $options = "";
    while ($row = mysqli_fetch_assoc($result)) {
        $options .= "<option value='{$row['T_id']}'>{$row['sub_class']}</option>";
    }

    // Return the HTML options as the response to the AJAX request
    echo $options;
...
<body>
    ...
    <div class="card">
        ...
        <form action="index.inc.php" method="post">
            ...
            <?php 
                echo "<select name='asset_class_select' id='asset_class_select' >";
                ...
                echo "<option hidden value=\"\">Select Asset Class</option>";
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                         echo "<option value=\"" . $row["ast_id"] . "\">" . $row["asset_class"] . "</option>";
                        //var_dump($row);
                    }
                } else {
...
<select id="asset_class_sub_classes" name="asset_class_sub_classes" required>
    ...
    assetClassSelect.addEventListener('change', function() {
        ...
        if (assetClass) {
            ...
            assetSubClassSelect.innerHTML = '<option>Select Asset Class First</option>';
                    ...
                    <input type="text" id="identificationNumber" name="identificationNumber" minlength="4"  maxlength="10" required oninput="capitalizeFirstLetter(this)" value="<?php echo htmlspecialchars($identificationNumber); ?>">
