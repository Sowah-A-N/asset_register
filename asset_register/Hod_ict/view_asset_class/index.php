...
if(isset($_POST['add']))
{
  ...
  if (!isset($name) || !isset($opening_balance) || !isset($depreciation) || !isset($estimated_life) || !isset($acc_dep_opening_balance)) {
    ...
    if (isset($row['count']) && $row['count'] > 0) {
        ...
        $insert_query = "INSERT INTO asset_classes (asset_class, account_depr_open_bal, Opening_bal, dep_rate, estimated_life) 
                         VALUES ('$name', '$acc_dep_opening_balance', '$opening_balance', '$depreciation', $estimated_life)";
        
        if ($conn->query($insert_query) === TRUE) {
            // Insert into asset_class_opbal_year
            $insert_opbal_query = "INSERT INTO asset_class_opbal_year (asset_class, year, opening_balance, total_accum_depr_start, expected_life_months, rate)
                                   VALUES ('$name', '$year', '$opening_balance', '$acc_dep_opening_balance', '$estimated_life_months', '$depreciation')";
                          ...
                          <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                              <div class="modal-content">
                                ...
                                <script>
                                      // Function to calculate and update estimated_life
                                      function updateEstimatedLife() {
                                          var depRate = parseFloat(document.getElementById('dep_rate').value);
                                          var estimatedLifeField = document.getElementById('estimated_life');
                                          
                                          // Check if depRate is a valid number
                                          if (!isNaN(depRate) && depRate !== 0) {
                                              var estimatedLife = Math.round(1 / depRate); // Calculate estimated_life without decimal places
                                              estimatedLifeField.value = estimatedLife; // Update the estimated_life field
                                          } else {
                                              estimatedLifeField.value = ''; // Clear estimated_life field if depRate is invalid or 0
                                          }
                                      }
                                  </script> 
                                ...
                                <div class="modal-body">
                                  ...
                                  <label for="depreciationRate">Depreciation Rate:</label>
                                    <input type="text" class="form-control" name="depreciation_rate" id="dep_rate"  placeholder="Enter Depreciation Rate" oninput="updateEstimatedLife()" required>
                                  </div>

                                  <div class="form-group">
                                  <label for="estimatedLife">Estimated life:</label>
                                    <input type="text" class="form-control" name="estimatedLife"  id="estimated_life" placeholder="" readonly>
                                  </div>
