...
$assetClass_total_accum_depr_start = $_SESSION['assetClassData']['asset_class_info']['total_accum_start_usd'];
$assetClass_total_depr_year_charge = $_SESSION['assetClassData']['asset_class_info']['total_depr_year_charge_usd'];
$assetClass_total_accum_depr_end = $_SESSION['assetClassData']['asset_class_info']['total_accum_end_usd'];
$assetClass_disposals_depreciation = $_SESSION['assetClassData']['asset_class_info']['disposals_depr_usd'];
$assetClass_total_net_book_value_end = $_SESSION['assetClassData']['asset_class_info']['net_book_value_usd'];
...
if (isset($assetClassData['monthly_depreciations']) && !empty($assetClassData['monthly_depreciations']) && is_array($assetClassData['monthly_depreciations'])) {
    ...
    foreach ($assetClassData['monthly_depreciations'] as $total) {
        ...
        $sheet->setCellValue($cell, number_format($total["value"]/$assetClass_rate, 2));
        $colIndex++;
    }
}

foreach ($assetData["assetsData"] as $asset) {
   $asset_rate =$asset["newAssetDollarRate"];
    $row = [
        $counter++,
        $asset["newAssetName"],
...
if (isset($assetClassData['monthly_depreciations']) && !empty($assetClassData['monthly_depreciations']) && is_array($assetClassData['monthly_depreciations'])) {
    ...
    foreach ($assetClassData['monthly_depreciations'] as $assetMonthlyTotal) {
        ...
        $monthlyGrandTotal = ($assetData["assetsTotals"]["monthlyTotals"][$monthIndex]) + ($assetMonthlyTotal["value"]/$assetClass_rate);
