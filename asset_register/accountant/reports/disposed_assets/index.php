              ...
              <!-- <a href='../generator/' target="_blank"><button class="btn bg-primary" type="button" id="dropdownMenuButton1" aria-haspopup="true" aria-expanded="false"> Generate Timetable</button></a> -->
                  ...
                  <?php
                    //echo "Page for " . $_GET['asset_class'];
                     ...
                     echo "<label for='asset_class_select' class='px-2 text-lg font-semibold'>Select Asset Class:</label>";
                     echo "<select name='asset_class_select' id='asset_class_select' class='px-4 p-2 border border-gray-300 rounded'>";         
                    ...
                    if (!isset($selectedAssetClass) || $selectedAssetClass == "" ) {
                      // Query 1: All Disposed Assets
                      $sqlClassQuery = "SELECT assets.*, asset_classes.dep_rate, asset_classes.account_depr_open_bal, asset_classes.estimated_life, asset_classes.opening_bal 
                                        ...
                                        WHERE assets.disposals > 0;";

                    } elseif (isset($selectedAssetClass)) {
                      //Query 2: All Disposed Assets selected asset class
                      $sqlClassQuery = "SELECT assets.*, asset_classes.dep_rate, asset_classes.account_depr_open_bal, asset_classes.estimated_life, asset_classes.opening_bal 
                      ...
                      exit;
                       ...
                       if($sqlClassResult === false){
                        echo "Error executing query : ". $conn -> error ;
                        die();

                       } elseif (mysqli_num_rows($sqlClassResult) > 0) { //if this user has some data in the database
                            ...
                            $totalDollarAdditions = $totalDepreciationCost = $totalDisposals = 0;
                                            
                            //  echo "<div class='h-screen overflow-auto mx-auto w-3/4 bg-white p-4 shadow-md rounded mt-4'>";
                            if(isset($selectedAssetClass)){
                                echo "<div class='h-screen overflow-auto mx-auto w-3/4 bg-white p-4 shadow-md rounded mt-4'>";
                                echo "<h2 class='font-san-serif font-bold text-lg'>Disposals for {$selectedAssetClass}</h2>";
                                } else {
                                echo "<div class='h-screen overflow-auto mx-auto w-3/4 bg-white p-4 shadow-md rounded mt-4'>";
                                echo "<h2 class='font-san-serif font-bold text-lg'>Disposals For All Assets</h2>";
                                }
                            ...
                            echo "<thead><tr class='bg-gray-200 '>
                                    ...
                                    <th class='py-2 px-4 border '>Disposal Value(GHC)</th>
                            ...
                            echo "<tbody>";                             
                                    ...
                                    while ($row = mysqli_fetch_assoc($sqlClassResult)) {
                                        ...
                                        // Now $accDeprOpeningBal contains the appropriate value
                                            ...
                                            <td class='border px-4 py-2'>". $row['sub_class']."</td>
                                            ...
                                            <td class='py-2 px-4 border-b'>" . number_format($row['disposals'], 2, ".", ",") . "</td>
                            ...
                            echo "<tr><td class='border px-4 py-2'>".""."</td>
                                    ...
                                    $totalDisposals = isset($totalDisposals) ? $totalDisposals : 0;                 
                            ...
                            echo "<tr><td class='border px-4 py-2 font-bold'>"."Totals"."</td>
                                ...
                                <td class='border px-4 py-2 font-bold text-blue-600'>".(isset($totalDisposals) ? number_format($totalDisposals, 2, ".", ",") :"")."</td> <!--Total Disposals -->
