...
if (isset($_POST['add'])) {
  ...
  $blacklist_reason = mysqli_real_escape_string($conn, $_POST['blacklist_reason']);

  if (empty($index_no) || empty($blacklist_reason)) {
    echo "<script> alert('The details submitted are not correct'); window.location='index.php' </script> ";
    exit();
  } else {
    //Check for duplication
    $query = "SELECT * FROM blacklist_students WHERE index_number='$index_no'";
    ...
    if ($result_check > 0) {
      ...
      VALUES ('$index_no', '$blacklist_reason', '$username')";
      ...
      echo "<script> alert('Student Added to Blacklist Successfully');  window.location='index.php';</script> ";
    }
  }
}
?>
    ...
    <div class="main-panel">
      <div class="content-wrapper">
        ...
        <div class="d-xl-flex justify-content-between align-items-start">
          <h2 class="text-dark font-weight-bold mb-2"> Add Students List </h2>
          <div class="d-sm-flex justify-content-xl-between align-items-center mb-2">
            <div class="dropdown ml-0 ml-md-4 mt-2 mt-lg-0">
              <button class="btn bg-danger" type="button" id="dropdownMenuButton1" aria-haspopup="true" aria-expanded="false" data-toggle="modal" data-target="#exampleModal"> Add Single Student to Blacklist</button>
            </div>
          </div>
        </div>
        <div class="row">
                      ...
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                          <form method="POST" action="index.php" name="add_student">
                            <div class="form-group">
                              <label for="exampleInputEmail1">Index Number of Student</label>
                              <input type="text" class="form-control" name="index_no" placeholder="Enter Student's Index No." required>
                            </div>
                            <div class="form-group">
                              <label for="exampleInputEmail1">Reason for Blacklisting</label>
                              <textarea class="form-control" rows="4" cols="50" name="blacklist_reason" required></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                          <button type="submit" class="btn btn-danger" name="add">Blacklist Student</button>
                          </form>
                        </div>
                      </div>
                    </div>
