...
$asset_name=$row["asset_class"];
  ...
  if (!isset($sub_class)||!isset($asset_class)||!isset($sub_code) )
  {
  ...
  else
  {
    ...
    $check_query_sub_class = "SELECT COUNT(*) as count FROM asset_class_sub_classes WHERE sub_class = '$sub_class'";
    $result_sub_class = $conn->query($check_query_sub_class);
    $row_sub_class = $result_sub_class->fetch_assoc();

    // SQL query to check if the sub_code already exists
    $check_query_sub_code = "SELECT COUNT(*) as count FROM asset_class_sub_classes WHERE sub_class_code = '$sub_code'";
    $result_sub_code = $conn->query($check_query_sub_code);
    $row_sub_code = $result_sub_code->fetch_assoc();

   if ( $row_sub_class['count'] > 0) {
    echo '<script type="text/javascript">alert("Sub Class already exists.");window.location=\'index.php\';</script>';
} 
elseif ($row_sub_code['count'] > 0) {
    echo '<script type="text/javascript">alert("Sub Code already exists.");window.location=\'index.php\';</script>';
}
    else {
       
      $insert_query = "INSERT INTO asset_class_sub_classes (sub_class, sub_class_code, asset_class ) 
      VALUES ('$sub_class', '$sub_code', '$asset_name')";
...
<table class="table">
    <thead>
        <tr>
            ...
            <th scope="col">Sub-Class Code</th>
                                ...
                                <div class="modal-body">
                                  ...
                                  <label for="sub_class">Name of Sub-Class:</label>
                                    <input type="text" class="form-control" name="sub_class"  id='sub_class'
                                     placeholder="Enter Name of Sub-class" required oninput="capitalizeFirstLetter(this)">
                                    
                                </div>

                                <div class="form-group">
                                  <label for="sub_code">Sub-Class Code:</label>
                                    <input type="text" class="form-control" name="sub_code"  id='sub_code'
                                     placeholder="Enter Sub class Code" required oninput="capitalizeFirstLetter(this)">
