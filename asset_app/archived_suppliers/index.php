<?php
/** FILE: schedule_officer/archived_suppliers/index.php — Archived suppliers (read-only), v2. */
session_start();
...
include "../datacon.php";
$rows = [];
$r = mysqli_query($conn, "SELECT name, location, number FROM suppliers_archive ORDER BY name ASC");
...
<head>
  ...
  <title>Archived Suppliers — RMU Asset Register</title>
...
<div class="rmu-layout">
  ...
  <div class="rmu-main">
    <header class="rmu-topnav">
      ...
      <div class="topnav-breadcrumb"><h1>Archived Suppliers</h1><small><?= count($rows) ?> archived</small></div>
      <div class="topnav-actions"><div class="dropdown">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
          ...
          <li><a class="dropdown-item" href="../supplier/"><i class="bi bi-truck me-2"></i>Active Suppliers</a></li>
    ...
    <main class="rmu-content">
      <div class="rmu-card" style="max-width:760px">
        <div class="rmu-card-header"><div class="rmu-card-title">Archived Suppliers</div></div>
        <div class="rmu-card-body" style="padding:0">
          <?php if (empty($rows)): ?>
            <div class="rmu-empty"><i class="bi bi-archive rmu-empty-icon"></i><h6>No archived suppliers</h6></div>
          <?php else: ?>
            <table class="rmu-table" style="box-shadow:none;border:none">
              <thead><tr><th>S/N</th><th>Name</th><th>Location</th><th>Contact</th></tr></thead>
              <tbody>
                <?php $sn=1; foreach ($rows as $s): ?>
                  <tr><td><?= $sn++ ?></td><td><?= htmlspecialchars($s['name']) ?></td><td><?= htmlspecialchars($s['location']) ?></td><td><?= htmlspecialchars($s['number']) ?></td></tr>
                <?php endforeach; ?>
              </tbody>
            </table>
          <?php endif; ?>
    ...
    <footer style="padding:.85rem 1.75rem;border-top:1px solid var(--card-border);background:white;font-size:.75rem;color:var(--text-muted);display:flex;justify-content:space-between;flex-wrap:wrap;gap:.5rem">
      <span>RMU Asset Register &nbsp;·&nbsp; Archived Suppliers</span><span><?= date('Y') ?> &nbsp;·&nbsp; Regional Maritime University</span>
    </footer>
  </div>
</div>
