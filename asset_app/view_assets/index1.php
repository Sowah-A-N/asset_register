...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'archive' && isset($_POST['asset_id'])) {
    ...
    if ($ok1 && $ok2) { mysqli_commit($conn); echo "<script>alert('Asset archived'); window.location='index1.php';</script>"; exit; }
    else { mysqli_rollback($conn); echo "<script>alert('Archive failed'); window.location='index1.php';</script>"; exit; }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'move' && isset($_POST['serial_number'])) {
    ...
    $notes = $_POST['notes'] ?? '';
    if (empty($new_location) || empty($new_user) || empty($notes)) { echo "<script>alert('Check details'); window.location='index1.php';</script>"; exit; }
    ...
    if (!$gres || mysqli_num_rows($gres) === 0) { mysqli_stmt_close($gs); echo "<script>alert('Asset not found'); window.location='index1.php';</script>"; exit; }
    $crow = mysqli_fetch_assoc($gres);
    mysqli_stmt_close($gs);
    if ($crow['user'] === $new_user) { echo "<script>alert('Users are the same'); window.location='index1.php';</script>"; exit; }
    ...
    else { echo "<script>alert('Error changing location'); window.location='index1.php';</script>"; exit; }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit' && isset($_POST['serial_number'])) {
    ...
    if ($ok) {
        echo "<script>alert('Asset updated'); window.location='index1.php';</script>";
        exit;
    } else {
        echo "<script>alert('Update failed: " . esc(mysqli_error($conn)) . "'); window.location='index1.php';</script>";
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'dispose' && isset($_POST['asset_id'])) {
    ...
    if (!$fres || mysqli_num_rows($fres) === 0) { echo "<script>alert('Asset not found'); window.location='index1.php';</script>"; exit; }
    ...
    if ($disposalValue < 0) { echo "<script>alert('Disposal value cannot be negative'); window.location='index1.php';</script>"; exit; }
    $currentYear = date('Y');
    ...
    mysqli_stmt_close($ins);
    if (!$ok) { echo "<script>alert('Disposal insert failed'); window.location='index1.php';</script>"; exit; }
    ...
    else { echo "<script>alert('Failed to update disposal flag'); window.location='index1.php';</script>"; exit; }
}
...
<body>
  ...
  <div class="container-scroller">
    ...
    <div class="main-panel">
     ...
     <form method="GET" action="index1.php" style="margin-bottom:15px;">
      ...
      ?>
        <tr>
          ...
          <td>
            ...
            <form style="display:inline" method="POST" action="index1.php" onsubmit="return confirm('Archive this asset?')">
              <input type="hidden" name="action" value="archive">
              <input type="hidden" name="asset_id" value="<?= esc($row['asset_id']) ?>">
              <button type="submit" class="btn btn-danger btn-sm">Archive</button>
            </form>
            <form style="display:inline" method="POST" action="index1.php" onsubmit="return confirm('Dispose this asset?')">
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editForm" method="POST" action="index1.php">
        ...
        <div class="modal fade" id="moveModal" tabindex="-1" role="dialog" aria-hidden="true">
          <div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Move Asset</h5><button type="button" class="close" data-dismiss="modal">&times;</button></div>
              <div class="modal-body">
                <form id="moveForm" method="POST" action="index1.php">
  ...
  <script>
    ...
    function openEditModal(serial) {
      fetch('index1.php?action=get_asset&asset_id=' + encodeURIComponent(serial))
        .then(r => r.json())
        .then(data => {
